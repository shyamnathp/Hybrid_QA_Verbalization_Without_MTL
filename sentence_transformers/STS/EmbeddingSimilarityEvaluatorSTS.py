...
class EmbeddingSimilarityEvaluatorSTS(SentenceEvaluator):
    """
    ...
    def __call__(self, model: 'SequentialSentenceEmbedder', output_path: str = None, epoch: int = -1, steps: int = -1) -> float:
        ...
        self.dataloader.collate_fn = model.smart_batching_collate_sts
        ...
        for step, batch in enumerate(iterator):
            sts = 1
            features, label_ids, target = batch_to_device(batch, sts, self.device)
            with torch.no_grad():
            ...
            embeddings1.extend(emb1.to("cpu").numpy())
            embeddings2.extend(emb2.to("cpu").numpy())
