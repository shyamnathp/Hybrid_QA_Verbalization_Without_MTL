...
def batch_to_device(batch, sts, target_device: device):
    """
    ...
    if(not sts):
        ...
        #labels = torch.tensor(labels, dtype=torch.int64).to(target_device)
        targets = batch['targets'].to(target_device)
        #targest = torch.stack(targets)
    else:
        labels = batch['labels'].to(target_device)
        targets = torch.empty([batch['labels'].size()[0]]).to(target_device)

    # for label_idx in range(len(labels)):
    #     labels[label_idx] = labels[label_idx].to(target_device)
