...
Since we have a hyrid model, we define custom masking. The rest of the parts remain the same.
The encoder input to the decoder is combined encoder_question and encoder_query
"""
import math
import torch
...
class Decoder(nn.Module):
    ...
    def forward(self, trg_tokens, encoder_out, **kwargs):
        ...
        for layer in self.layers:
            x, attention = layer(x, encoder_out, trg_mask, src_mask)

        return self.linear_out(x), attention
...
class DecoderLayer(nn.Module):
    ...
    def forward(self, embed_trg, embed_src, trg_mask, src_mask):
        ...
        """
        #changed
        _trg, _ = self.self_attn(embed_trg, embed_trg, embed_trg, trg_mask)
        x = self.layer_norm(embed_trg + self.dropout(_trg))
        _trg, attention = self.src_attn(x, embed_src, embed_src, src_mask)
        x = self.layer_norm(x + self.dropout(_trg))
        x = self.layer_norm(x + self.dropout(self.pos_ff(x)))
...
class MultiHeadedAttention(nn.Module):
    ...
    def forward(self, query, key, value, mask=None):
        ...
        #changed
        attentionOne = F.softmax(energy, dim=-1) # (batch, heads, sent_len, sent_len)
        attention = F.dropout(attentionOne, p=self.dropout, training=self.training)
