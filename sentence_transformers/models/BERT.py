...
class BERT(nn.Module):
    ...
    def __init__(self, model_name_or_path: str, max_seq_length: int = 128, do_lower_case: bool = True):
        ...
        self.tokenizer = DistilBertTokenizer.from_pretrained('/data/premnadh/Hybrid-QASystem/sentence_transformers/Vocab/DistilBert_Vocab.txt', do_lower_case=do_lower_case)
    ...
    def tokenize(self, text: str) -> List[int]:
        ...
        """
        test = self.tokenizer.convert_tokens_to_ids('ent1')
    ...
    def pad_targets(self, tokens: List[int], pad_seq_length: int):
        tokens = [self.cls_token_id] + tokens + [self.sep_token_id]
        pad_seq_length += 2
        padding = [0] * (pad_seq_length - len(tokens))
        target = tokens + padding
        return target

    def fc_layer(self, cosinescore_tensor):
        return self.fc(cosinescore_tensor)
