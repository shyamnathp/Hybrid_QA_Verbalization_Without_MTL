...
class SentencesDataset(Dataset):
    ...
    def convert_input_examples(self, examples: List[InputExample], model: SentenceTransformer):
        ...
        for ex_index, example in enumerate(iterator):
            ...
            tokenized_target = model.tokenize(example.label)
        ...
        tensor_targets = torch.tensor(targets, dtype=torch.float)
        ...
        self.tokens = inputs
        self.labels = labels
        self.targets = tensor_targets

    def __getitem__(self, item):
        return [self.tokens[i][item] for i in range(len(self.tokens))], self.labels[item], self.targets[item]
